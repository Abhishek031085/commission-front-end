...
const SearchCommission = () => {
  const [filters, setFilters] = useState({ state: '', month: '', policyType: '', segment: '' });
  const [dropdowns, setDropdowns] = useState({ months: [], policyTypes: [], segments: [] });
  const [results, setResults] = useState([]);

  //load states
  useEffect(() => {
    axios.get(`${API}/api/states`).then(res =>
      setDropdowns(prev => ({ ...prev, states: res.data.map(item => item.state) }))
    );
  }, []);
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/months?state=${filters.state}`).then(res =>
      setDropdowns(prev => ({ ...prev, months: res.data.map(item => item.month) }))
    );
    setFilters(f => ({ ...f, month: '', policyType: '', segment: '' }));
    setDropdowns(prev => ({ ...prev, policyTypes: [], segments: [] }));
  }, [filters.state]);

  // Load policy types when month changes
  useEffect(() => {
    if (!filters.month) return;
    axios.get(`${API}/api/policy_types?month=${filters.month}&state=${filters.state}`).then(res =>
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API}/api/segments?month=${filters.month}&policyType=${filters.policyType}&state=${filters.state}`)
    ...
    <div className="container mt-5">
      ...
      <div className="row mb-3">
        <div className="col-md-2 mb-2">
          <select className="form-select" name="state" onChange={handleChange} value={filters.state}>
            <option value="">Select State</option>
            {dropdowns.states?.map((s, i) => (
